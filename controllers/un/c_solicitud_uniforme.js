...
// paquete 
    ...
    async gettipo_entregass(){  
        ...
        const response = await this.request('../../models/un/bd_tipo_entregas.php',{'order' : 'ORDER BY id_tipo_entrega DESC','action' : 'select','filter' : filtrarPor});
        try{ 
...
//detalle  
        ...
        async delete_solicitud_uniforme(id_solicitud_uniforme){
            ...
            if(id_solicitud_uniforme > 0){
                const response = await this.request(this.path,{model:{'id_solicitud_uniforme':id_solicitud_uniforme},'action' : 'delete'});
                if(response.message == 'Data Deleted'){
        ...
        async save_solicitud_uniforme(){ 
            if(this.validar_solicitud()){ 
                ...
                if(this.solicitud_uniforme.id_solicitud_uniforme > 0){
                    ...
                    // --
                    this.solicitud_uniforme.total = Number(monto).toFixed(2);
                    this.solicitud_uniforme.estado = 'CO';
                    const response = await this.request(this.path,{model:this.solicitud_uniforme,'action' : 'update'});
                    if(response.message == 'Data Updated'){
                    ...
                    const response = await this.request(this.path,{model:this.solicitud_uniforme,'action' : 'insert'});
        ...
        search_solicitud_uniformeByID(id_solicitud_uniforme){
            ...
            this.solicitud_uniforme = {id_solicitud_uniforme:0,id_empleado:this.account.id_empleado,estado:'',total:'0.00'};
        },
        ...
        async fill_f_keys(){
            ...
            // console.log(response_numsZapato);
            // console.log(response_tallas);
            try{  
                if(response_tallas.length > 0 && response_tallas[0].id_talla > 0){  
