...
var application = new Vue({
    ...
    methods:{
        ...
        async getevConsulEstado(){  
            ...
            this.getEstadosCombo2();
        }, 

        async getEstadosCombo(){
            this.ev_propuestaCollection  = [];
            this.paginaCollection = [];
            const response = await this.request(this.path,{'action' : 'selectConEstado','filterestado' : this.filterestado, model:this.ev_propuesta});
        ...
        async getEstadosCombo2(){
            this.ev_propuestaCollection2  = [];
            this.paginaCollection2 = [];
            const response2 = await this.request(this.path,{'action' : 'selectConEstado2','filterestado' : this.filterestado, model:this.ev_propuesta});
            console.log('Proactivo');
            console.log(response2);
            try{ 
                this.show_message(response2.length + ' Registros Encontrados.','success');
                this.ev_propuestaCollection2 = response2;
                this.paginaCollection2 = response2;
                this.paginator(1);  
                this.isFormCrud=false;
            }catch(error){
                this.show_message('No hay datos Para Mostrar.','info');
        ...
        async getev_propuestas(){  
            this.ev_propuestaCollection = [];
            this.paginaCollection = [];
            const response = await this.request(this.path,{model:this.ev_propuesta, 'action' : 'select'});
            try{ 
                this.show_message(' Registros Encontrados.','success');
                this.ev_propuestaCollection = response;
                this.paginaCollection = response;
                this.paginator(1);  
                this.isFormCrud=false;
        ...
        async getev_propuestas2(){  
            this.ev_propuestaCollection2 = [];
            this.paginaCollection2 = [];
            const response = await this.request(this.path,{model:this.ev_propuesta, 'action' : 'selected'});
           
            // const response = await this.request(this.path,{'action' : 'select','filter' : this.filter});
            try{ 
                this.show_message(' Registros Encontrados.','success');
                this.ev_propuestaCollection2 = response;
                this.paginaCollection2 = response;
                this.paginator(1);  
                this.isFormCrud=false;
            }catch(error){
                this.show_message('No hay datos Para Mostrar.','info');
        ...
        async update_ev_propuesta2(propuesta_id){ 
            if(propuesta_id > 0){
                this.ev_propuesta = this.search_ev_propuestaByID2(propuesta_id);
                if(this.ev_propuesta.propuesta_id > 0){
                    this.save_ev_propuesta();
                }else{
                    this.show_message('Hay un problema con este Registro.','info');
        ...
        async fill_f_keys(){
            ...
            let cantidad_pages = Math.ceil(this.ev_propuestaCollection2.length / this.numByPag);
            this.paginas = []; 
            if (i === 'Ant' ) {
                if (this.paginaActual == 1) {  i = 1;  }else{  i = this.paginaActual -1; } 
            }else if (i === 'Sig') { 
            ...
            for (let indexI = 0; indexI < cantidad_pages; indexI++) {
                ...
                if (indexI == (i - 1) ) { 
                    ...
                    let inicio = ( i == 1 ? 0 : ((i-1) *  parseInt(this.numByPag)));
                    inicio = parseInt(inicio);
                    let fin = (cantidad_pages == i ? this.ev_propuestaCollection2.length : (parseInt(inicio) + parseInt(this.numByPag)));  
                    for (let index = inicio; index < fin; index++) {
                        const element = this.ev_propuestaCollection2[index];
                        this.paginaCollection2.push(element); 
                    }  
                }  
            }  
            this.paginas.push({'element':'Sig'});
