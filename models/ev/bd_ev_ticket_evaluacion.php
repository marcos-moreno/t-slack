    ...
    public function save_dat_evaluac_por_user(){
        ...
        try {
            ...
            $parameters = array(
                ...
                ':v_ev_ticket_ln_id' => $this->received_data->ev_ticket_ln_id,
                ':v_solucionadopor' => $this->received_data->solucionadopor,
                ':v_id_user' => $_SESSION['id_empleado'],
            );
            $query = '
                SELECT refividrio.save_dat_evaluac_por_userticket(
                    :respuestas_collection,:v_ev_ticket_ln_id,:v_id_user, :v_solucionadopor
                )
            ';
            $statement = $this->connect->prepare($query); 
            $statement->execute($parameters);   
            ...
            $output = array('status' => 'error','message' => $exc,'params' => $parameters); 
            echo json_encode($output); 
            return false;
        }  
    }
