<?php  

 require_once '../../models/postgres.php';
 $received_data = json_decode(file_get_contents('php://input')); 
 $model = null;
 require_once '../../models/auth/check.php';

if (check_session()) { 
    switch($received_data->action){
        case 'update': 
            $model = new Compra_detalle($data,$connect,$received_data);
            $model->update();
        break;
        case 'insert':
            $model = new Compra_detalle($data,$connect,$received_data);
            $model->insert();
        break;
        case 'delete':
            $model = new Compra_detalle($data,$connect,$received_data);
            $model->delete(); 
        break;
        case 'select': 
            $model = new Compra_detalle($data,$connect,$received_data);
            $model->select();
        break;
    }
}else{
    $output = array('message' => 'Not authorized'); 
    echo json_encode($output); 
} 

class Compra_detalle 
{   
    
    private $output = null;
    private $data = array(); 
    private $connect = null;
    private $received_data = null;
    public function __construct($data,$connect,$received_data){
        $this->data  = $data;
        $this->connect = $connect;
        $this->received_data = $received_data;
    }

    public function insert(){
        try {
            $data = array(
                    ':id_compra' => $this->received_data->model->id_compra,
                            ':id_color' => $this->received_data->model->id_color,
                            ':id_talla' => $this->received_data->model->id_talla,
                            ':id_producto' => $this->received_data->model->id_producto,
                            ':total_linea' => $this->received_data->model->total_linea,
                            ':cantidad' => $this->received_data->model->cantidad,
                            ':id_almacen' => $this->received_data->model->id_almacen,
                            
                    ); 
        $query = 'INSERT INTO un_compra_detalle (id_compra,id_color,id_talla,id_producto,total_linea,cantidad,id_almacen) 
                    VALUES (:id_compra,:id_color,:id_talla,:id_producto,:total_linea,:cantidad,:id_almacen) ;';

            $statement = $this->connect->prepare($query); 
            $statement->execute($data);  
            $output = array('message' => 'Data Inserted'); 
            echo json_encode($output); 
            return true;
        } catch (Exception $exc) {
            $output = array('message' => $exc); 
            echo json_encode($output); 
            return false;
        } 
    } 

    public function update(){
        try {
            $data = array(
                    ':id_compra_detalle' => $this->received_data->model->id_compra_detalle, 
                            ':id_compra' => $this->received_data->model->id_compra, 
                            ':id_color' => $this->received_data->model->id_color, 
                            ':id_talla' => $this->received_data->model->id_talla, 
                            ':id_producto' => $this->received_data->model->id_producto, 
                            ':total_linea' => $this->received_data->model->total_linea, 
                            ':cantidad' => $this->received_data->model->cantidad, 
                            ':id_almacen' => $this->received_data->model->id_almacen, 
                            
                    ); 
        $query = 'UPDATE un_compra_detalle SET id_compra=:id_compra,id_color=:id_color,id_talla=:id_talla,id_producto=:id_producto,
        total_linea=:total_linea,cantidad=:cantidad,id_almacen=:id_almacen WHERE  id_compra_detalle = :id_compra_detalle ;';

            $statement = $this->connect->prepare($query); 
            $statement->execute($data);  
            $output = array('message' => 'Data Updated'); 
            echo json_encode($output); 
            return true;
        } catch (Exception $exc) {
            $output = array('message' => $exc); 
            echo json_encode($output); 
            return false;
        }  
    } 

    public function select(){
        try {  
             
        $query = 'SELECT id_compra_detalle,id_compra,id_color,id_talla,id_producto,total_linea,cantidad,id_almacen 
                    FROM un_compra_detalle  
                    ' . (isset($this->received_data->filter) ? ' 
                    WHERE ' . $this->received_data->filter:'') . 
                    (isset($this->received_data->order) ? $this->received_data->order:'') ;
                        
            $statement = $this->connect->prepare($query); 
            $statement->execute($data);   
            while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {  
                $row['compra'] = $this->search_union($row,'un_compra','id_compra','id_compra'); 
                $row['color'] = $this->search_union($row,'un_color','id_color','id_color'); 
                $row['talla'] = $this->search_union($row,'un_talla','id_talla','id_talla'); 
                $row['producto'] = $this->search_union($row,'un_producto','id_producto','id_producto');
                $row['almacen'] = $this->search_union($row,'un_almacen','id_almacen','id_almacen');
                $data[] = $row;
            }

        
            echo json_encode($data); 
            return true;
        } catch (Exception $exc) {
            $output = array('message' => $exc); 
            echo json_encode($output); 
            return false;
        }  
    }
    
    public function search_union($row,$table_origen,$fk_table_origen,$fk_table_usage){
        $data = array(); 
        try {    
            $query = 'SELECT * FROM '. $table_origen . '   WHERE '. $fk_table_origen . ' = ' .$row[$fk_table_usage] ;               
            $statement = $this->connect->prepare($query); 
            $statement->execute($data);   
            while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {   
                    $data[] = $row;
            }  
            return $data; 
        } catch (Exception $exc) {
            $output = array('message' => $exc); 
            echo json_encode($output); 
            return false;
        }  
    }
    public function delete(){
        try {  
            $data = array(
                   ':id_compra_detalle' => $this->received_data->model->id_compra_detalle,
                            
                    ); 
        $query = 'DELETE FROM un_compra_detalle WHERE id_compra_detalle = :id_compra_detalle ;'; 

            $statement = $this->connect->prepare($query); 
            $statement->execute($data);  
            $output = array('message' => 'Data Deleted'); 
            echo json_encode($output); 
            return true;
        } catch (Exception $exc) {
            $output = array('message' => $exc); 
            echo json_encode($output); 
            return false;
        }  
    }

  
    

} 